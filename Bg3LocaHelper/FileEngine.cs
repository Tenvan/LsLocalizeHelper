...
internal class FileEngine
{
  ...
  )
  {
    ...
    using var form        = new InputBoxForm("Set new file name", newFileName);
  ...
  )
  {
    ...
    if (result != DialogResult.OK
        && result != DialogResult.Yes) return false;
  ...
  {
    var       newFileName = Path.GetFileNameWithoutExtension(fileName);
    using var form        = new InputBoxForm("Input language name", "German");
    var       result      = form.ShowDialog();
