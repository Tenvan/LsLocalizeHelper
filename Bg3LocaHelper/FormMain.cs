...
public partial class FormMain : Form
{
  ...
  private static string GetCellValue(DataGridViewRow row, GridColumns column) =>
    row.Cells[(int)column]
       .Value.ToString();

  private static string? GetTextFromSource(object sender)
  {
  ...
  private static XmlNode? SelectNode(XmlNode? doc, object keySource, object versionSource)
  {
  ...
  private static void UpdateRowStatus(DataGridViewRow row, string newStatus)
  {
  ...
  private string CurrentModName => this.comboBoxMods.Text;
  ...
  private void buttonCopyFromOrigin_Click(object sender, EventArgs e)
  {
    Clipboard.SetText(this.textBoxCurrentOriginText.Text);
  }

  private void buttonCopyFromPrevious_Click(object sender, EventArgs e)
  {
    Clipboard.SetText(this.textBoxPreviousOriginText.Text);
  }

  private void buttonPasteToTranslated_Click(object sender, EventArgs e)
  {
    ...
    this.UpdateDataText(row: row, newText: newText);
    this.UpdateRowStatus();
    this.RecalcRowsAndColumnSizesHeights();
  }

  private void buttonRefreshMods_Click(object sender, EventArgs e)
  {
    this.RefreshMods();
  }

  private void checkBoxAutoClipboard_CheckedChanged(object sender, EventArgs e) { }

  private void comboBoxMods_SelectedValueChanged(object sender, EventArgs e)
  {
  ...
  private void comboBoxOriginCurrentFile_SelectedValueChanged(object sender, EventArgs e)
  {
    Settings.Default.pathOriginCurrent = this.comboBoxOriginCurrentFile.Text;
    Settings.Default.Save();
  }

  private void comboBoxOriginPreviousFile_SelectedValueChanged(object sender, EventArgs e)
  {
    Settings.Default.pathOriginPrevious = this.comboBoxOriginPreviousFile.Text;
    Settings.Default.Save();
  }

  private void comboBoxTranslatedFile_SelectedValueChanged(object sender, EventArgs e)
  {
  ...
  private void copyFileToolStripMenuItem_Click(object sender, EventArgs e)
  {
    ...
    var fileHelper = new FileEngine(modsPath: Settings.Default.pathMods, modeName: this.comboBoxMods.Text);

    if (fileHelper.CopyFile(fileName: fileName))
    {
      this.RefreshMods();
    }
  }

  private void copyFileToOtherLanguageToolStripMenuItem_Click(object sender, EventArgs e)
  {
    var fileName = FormMain.GetTextFromSource(sender);
    ...
    if (fileHelper.CopyFileToNewLanguage(fileName: fileName))
    {
      this.RefreshMods();
    }
  }
  ...
  private void dataGridViewSource_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
  {
    ...
    e.Handled = true;
    e.PaintBackground(clipBounds: e.CellBounds, cellsPaintSelectionBackground: true);
    var cellText = e.FormattedValue.ToString();
    var isSelected = (e.State & DataGridViewElementStates.Selected) != 0;

    var fontBrush = isSelected
                      ? new SolidBrush(e.CellStyle.SelectionForeColor)
                      : new SolidBrush(e.CellStyle.ForeColor);

    var pointF = new PointF(x: e.CellBounds.X + 2, y: e.CellBounds.Y + 2);
  ...
  private void dataGridViewSource_RowEnter(object sender, DataGridViewCellEventArgs e)
  {
    var dataIndex = e.RowIndex;
    var row = (sender as DataGridView)!.Rows[dataIndex];

    var keySource = row.Cells[(int)GridColumns.Uuid]
                       .Value.ToString();

    var versionSource = row.Cells[(int)GridColumns.Version]
                           .Value.ToString();

    var textSource = row.Cells[(int)GridColumns.Text]
                        .Value.ToString();
    ...
    {
      this.textBoxTranslatedText.Text = "";
    }

    var currentOriginNode = this.OriginCurrentDoc?.SelectSingleNode(xpath);

    this.textBoxCurrentOriginText.Text = currentOriginNode != null
                                           ? currentOriginNode.InnerText
                                           : "";

    var previousOriginNode = this.OriginPreviousDoc?.SelectSingleNode(xpath);

    this.textBoxPreviousOriginText.Text = previousOriginNode != null
                                            ? previousOriginNode.InnerText
                                            : "";
  }

  private void dataGridViewSource_RowPrePaint(object sender, DataGridViewRowPrePaintEventArgs e)
  {
  ...
  private void deleteFileToolStripMenuItem_Click(object sender, EventArgs e)
  {
    ...
    if (fileHelper.DeleteFile(fileName: fileName))
    {
      this.RefreshMods();
    }
  }
  ...
  private void importModToolStripMenuItem_Click(object sender, EventArgs e)
  {
    ...
    if (dialogResult != DialogResult.OK
        ...
        || string.IsNullOrWhiteSpace(formImport.ModName))
    {
      return;
    }

    var packer = new UnpackageEngine(
      modsPath: Settings.Default.pathMods,
      pakPath: formImport.PakToImport,
      modName: formImport.ModName
    );
  ...
  private void packingModToolStripMenuItem_Click(object sender, EventArgs e)
  {
    this.PackingMod();
  }
  ...
  private void renameFileToolStripMenuItem_Click(object sender, EventArgs e)
  {
    ...
    if (fileHelper.RenameFile(fileName: fileName))
    {
      this.RefreshMods();
    }
  }

  private void saveSourceXMLToolStripMenuItem_Click(object sender, EventArgs e)
  {
    this.SaveData();
  }
  ...
  private void textBoxTranslatedText_Enter(object sender, EventArgs e)
  {
  ...
  private void textBoxTranslatedText_Leave(object sender, EventArgs e)
  {
    var row = this.dataGridViewSource.CurrentRow;
  ...
  private void UpdateDataText(DataGridViewRow row, string newText)
  {
    row.Cells[(int)GridColumns.Text].Value = newText;

    foreach (DataRow dataRow in this.DataTable.Rows)
    {
      var dataUid = dataRow[(int)GridColumns.Uuid]
       .ToString();

      var rowUid = row.Cells[(int)GridColumns.Uuid]
                      .FormattedValue?.ToString();

      var dataVersion = dataRow[(int)GridColumns.Version]
       .ToString();

      var rowVersion = row.Cells[(int)GridColumns.Version]
                          .FormattedValue?.ToString();
